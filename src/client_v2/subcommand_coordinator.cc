...
void SetUpSubcommandGetCoordinatorMap(CLI::App &app) {
  ...
  coor->add_option("--coor_url", opt->coor_url, "Coordinator url, default:file://./coor_list")
      ->group("Coordinator Manager Commands");
  coor->add_option("--get_coordinator_map", opt->get_coordinator_map, "Request patameter get_coordinaotor_map")
      ->group("Coordinator Manager Commands");
...
void SetUpSubcommandQueryRegion(CLI::App &app) {
  ...
  auto coor = app.add_subcommand("QueryRegion", "Query region ")->group("Coordinator Manager Commands");
  coor->add_option("--coor_url", opt->coor_url, "Coordinator url, default:file://./coor_list")
      ->group("Coordinator Manager Commands");
  coor->add_option("--id", opt->id, "Request parameter region id ")->required()->group("Coordinator Manager Commands");
...
void SetUpSubcommandCreateRegion(CLI::App &app) {
  ...
  coor->add_option("--replica", opt->replica, "Request parameter replica num, must greater than 0")
      ->required()
      ->group("Coordinator Manager Commands");
  // yjddebug todo rawengine must rocksdb|bdb|xdp
...
void RunSubcommandCreateRegion(CreateRegionOption const &opt) {
  ...
  if (opt.raw_engine != "rocksdb" && opt.raw_engine != "bdb" && opt.raw_engine != "xdp") {
    ...
    std::cout << "raw_engine must be rocksdb, bdb or xdp";
    return;
  }
  ...
  if (opt.create_document_region) {
    ...
    if (opt.start_id > 0 && opt.end_id > 0 && opt.start_id < opt.end_id) {
      ...
      start_key = dingodb::Helper::EncodeDocumentIndexRegionHeader(opt.region_prefix.at(0), opt.part_id, opt.start_id);
      end_key = dingodb::Helper::EncodeDocumentIndexRegionHeader(opt.region_prefix.at(0), opt.part_id, opt.end_id);
    } else {
      DINGO_LOG(INFO) << "use part_id " << opt.part_id << " to create region range";
      start_key = dingodb::Helper::EncodeDocumentIndexRegionHeader(opt.region_prefix.at(0), opt.part_id);
      end_key = dingodb::Helper::EncodeDocumentIndexRegionHeader(opt.region_prefix.at(0), opt.part_id + 1);
    }
    ...
    if (opt.start_id > 0 && opt.end_id > 0 && opt.start_id < opt.end_id) {
      DINGO_LOG(INFO) << "use start_id " << opt.start_id << ", end_id " << opt.end_id << " to create region range";
      start_key = dingodb::Helper::EncodeVectorIndexRegionHeader(opt.region_prefix.at(0), opt.part_id, opt.start_id);
      end_key = dingodb::Helper::EncodeVectorIndexRegionHeader(opt.region_prefix.at(0), opt.part_id, opt.end_id);
    } else {
      DINGO_LOG(INFO) << "use part_id " << opt.part_id << " to create region range";
      start_key = dingodb::Helper::EncodeVectorIndexRegionHeader(opt.region_prefix.at(0), opt.part_id);
      end_key = dingodb::Helper::EncodeVectorIndexRegionHeader(opt.region_prefix.at(0), opt.part_id + 1);
    }
    DINGO_LOG(INFO) << "region range is : " << dingodb::Helper::StringToHex(start_key) << " to "
                    << dingodb::Helper::StringToHex(end_key);
    ...
    if (opt.metrics_type == "L2" || opt.metrics_type == "l2") {
      ...
      std::cout << "metrics_type is invalid, now only support L2, IP and COSINE";
      return;
    }
...
void SetUpSubcommandSplitRegion(CLI::App &app) {
  ...
  auto coor = app.add_subcommand("SplitRegion", "Split region ")->group("Coordinator Manager Commands");
  coor->add_option("--coor_url", opt->coor_url, "Coordinator url, default:file://./coor_list")
      ->group("Coordinator Manager Commands");
  ...
  coor->add_option("--split_from_id", opt->split_from_id, "Request parameter split from region id")
      ->required()
      ->group("Coordinator Manager Commands");
  coor->add_option("--split_key", opt->split_key, "Request parameter split_water_shed_key")
      ->group("Coordinator Manager Commands");
  coor->add_option("--vector_id", opt->vector_id, "Request parameter vector_id")->group("Coordinator Manager Commands");
  ...
  if (!opt.split_key.empty()) {
    ...
    if (query_response.region().definition().index_parameter().index_type() ==
      ...
      if (opt.vector_id > 0) {
        ...
        dingodb::VectorCodec::EncodeVectorKey(start_key[0], partition_id, opt.vector_id, real_mid);
      } else {
      ...
      if (opt.document_id > 0) {
        ...
        dingodb::DocumentCodec::EncodeDocumentKey(start_key[0], partition_id, opt.document_id, real_mid);
      } else {
...
void SetUpSubcommandMergeRegion(CLI::App &app) {
  ...
  auto coor = app.add_subcommand("MergeRegion", "Merge region ")->group("Coordinator Manager Commands");
  coor->add_option("--coor_url", opt->coor_url, "Coordinator url, default:file://./coor_list")
      ->group("Coordinator Manager Commands");
...
void SetUpSubcommandAddPeerRegion(CLI::App &app) {
  ...
  auto coor = app.add_subcommand("AddPeerRegion", "Add peer region ")->group("Coordinator Manager Commands");
  coor->add_option("--coor_url", opt->coor_url, "Coordinator url, default:file://./coor_list")
      ->group("Coordinator Manager Commands");
...
void SetUpSubcommandRemovePeerRegion(CLI::App &app) {
  ...
  auto coor = app.add_subcommand("RemovePeerRegion", "Remove peer region")->group("Coordinator Manager Commands");
  coor->add_option("--coor_url", opt->coor_url, "Coordinator url, default:file://./coor_list")
      ->group("Coordinator Manager Commands");
  coor->add_option("--store_id", opt->store_id, "Request parameter store id ")
      ->required()
...
void SetUpSubcommandGetTaskList(CLI::App &app) {
  ...
  coor->add_option("--coor_url", opt->coor_url, "Coordinator url, default:file://./coor_list")
      ->group("Coordinator Manager Commands");
  coor->add_option("--id", opt->id, "Request parameter task id")->required()->group("Coordinator Manager Commands");
...
void SetUpSubcommandBalanceLeader(CLI::App &app) {
  ...
  coor->add_option("--dryrun", opt->dryrun, "Request parameter safe dryrun")
      ->default_val(true)
      ->group("Coordinator Manager Commands");
  coor->add_option("--store_type", opt->store_type, "Request parameter store_type")
      ->required()
      ->group("Coordinator Manager Commands");
