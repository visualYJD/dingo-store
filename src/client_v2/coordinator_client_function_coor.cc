...
//                     << " response_attachment=" << cntl.response_attachment().size() << " latency=" <<
//                     cntl.latency_us();
...
//               << " epoch=" << region.definition().epoch().conf_version() << "," <<
...
//                     << " deleted_time_minutes=" << (butil::gettimeofday_ms() - region.deleted_timestamp()) / 1000 /
//                     60
//                     << " deleted_time_hours=" << (butil::gettimeofday_ms() - region.deleted_timestamp()) / 1000 /
//                     3600;
//   }
...
//     }
//     DINGO_LOG(INFO) << fmt::format("store_id={},region_count={},region_ids={}", it.id(),
//     it.region_metrics_map_size(),
//                                    region_ids);
//   }
// }
...
//       DINGO_LOG(INFO) << "use start_id " << FLAGS_start_id << ", end_id " << FLAGS_end_id << " to create region
//       range"; start_key =
...
//         R"({"col1": { "tokenizer": { "type": "chinese"}}, "col2": { "tokenizer": {"type": "i64", "indexed": true }},
//         "col3": { "tokenizer": {"type": "f64", "indexed": true }}, "col4": { "tokenizer": {"type": "chinese"}} })";
...
//       end_key = dingodb::Helper::EncodeVectorIndexRegionHeader(FLAGS_region_prefix.at(0), FLAGS_part_id,
//       FLAGS_end_id);
//     } else {
//       DINGO_LOG(INFO) << "use part_id " << FLAGS_part_id << " to create region range";
...
//   auto status2 =
//       coordinator_interaction->SendRequest("TransferLeaderRegion", transfer_leader_request,
//       transfer_leader_response);
//   DINGO_LOG(INFO) << "SendRequest status=" << status2;
